...
LRESULT CALLBACK windowProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
  switch (message) {
    ...
    case WM_WINDOWPOSCHANGED: {
      const WINDOWPOS* position = reinterpret_cast<const WINDOWPOS*>(lParam);
      // During an interactive move the client surface itself does not change.
      // Repainting it on every position update repeatedly clears/rebuilds the
      // DWM titlebar and synchronously flushes composition, which can show up
      // as a subtle flicker. Let DWM move the existing surface for pure moves,
      // but keep the refresh path for resize, DPI/frame and Z-order updates.
      const bool pureMove =
          (position->flags & SWP_NOSIZE) != 0 &&
          (position->flags & SWP_NOMOVE) == 0 &&
          (position->flags & SWP_FRAMECHANGED) == 0;
      if (!pureMove) postFrameRefresh(hwnd);
